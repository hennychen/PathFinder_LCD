                               ...
                               bool swap_color, esp_eaf_palette_cache_t *cache)
{
    ...
    /* Empty decode result (transparent/uniform block) — leave buffer as zeros */
    if (out_size == 0 && !is_jpeg) {
        if (alpha_block != NULL) {
            memset(alpha_block, 0, pixel_count);
        }
        free(offsets);
        if (work_buffer) {
            free(work_buffer);
                                 ...
                                 bool swap_color)
{
    ...
    if (encoded_size == 0) {
        /* Empty block — valid for transparent/uniform regions */
        *out_size = 0;
        return ESP_OK;
    }
