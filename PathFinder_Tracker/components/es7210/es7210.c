...
struct es7210_dev_t {
    i2c_master_dev_handle_t i2c_dev;  /*!< I2C device handle from new master driver */
    uint8_t     i2c_addr;
};
...
static esp_err_t es7210_write_reg(es7210_dev_handle_t handle, uint8_t reg_addr, uint8_t reg_val)
{
    ...
    ESP_RETURN_ON_ERROR(i2c_master_transmit(handle->i2c_dev, write_buf, sizeof(write_buf), -1),
                        TAG, "i2c communication error while writing register");
...
esp_err_t es7210_new_codec(const es7210_i2c_config_t *i2c_conf, es7210_dev_handle_t *handle_out)
{
    ...
    /* Add ES7210 as a device on the I2C master bus */
    ...
    esp_err_t ret = i2c_master_bus_add_device(i2c_conf->i2c_bus_handle, &dev_cfg, &handle->i2c_dev);
    if (ret != ESP_OK) {
        free(handle);
        return ret;
    }
...
esp_err_t es7210_config_codec(es7210_dev_handle_t handle, const es7210_codec_config_t *codec_conf)
{
    ...
    /* Keep ADC clocks on (safe for slave mode; REG01 default=0x20 turns off master clocks) */
    ES7210_WRITE_REG(ES7210_CLOCK_OFF_REG01, 0x00);
    /* Disable ADC auto-mute (redundant, default=0x00, but explicit is safe) */
    ES7210_WRITE_REG(ES7210_ADC_AUTOMUTE_REG13, 0x00);
    /* NOTE: Do NOT touch REG08 unless using multiple-LRCK TDM mode.
     * REG08 bit2 = EQ_OFF (not channel count). 1xFS TDM is controlled via REG12 alone. */
    /* Enable device */
