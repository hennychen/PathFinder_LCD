...
//===== 外部中断配置函数 =====//
//GPIOx:0~4,代表GPIOA~E
//GPIO_Pin:需要使能GPIO位，范围 GPIO_Pin_0~GPIO_Pin_15
//TRIM:触发模式,1,下降沿;2,上升沿;3，双边沿触发
//该函数一次只能配置1个IO口。多个IO口,需多次调用
//该函数会自动开启对应中断,以及屏蔽线   	
void Ex_NVIC_Config(GPIO_TypeDef* GPIOx,uint32_t GPIO_Pin,u8 TRIM) 
{ 
	u8 EXTOFFSET,i; 
	u32 n=((u32)GPIOx)/0x0400;
	RCC->APB2ENR|=1<<14;  						//使能SYSCFG时钟
	for(i=0;i<15;i++)if(GPIO_Pin&(1<<i))break;
	EXTOFFSET=(i%4)*4;
	SYSCFG->EXTICR[i/4]&=~(0x000F<<EXTOFFSET);//清除原来设置！！！
	SYSCFG->EXTICR[i/4]|=n<<EXTOFFSET;	//EXTI.BITx映射到GPIOx.BITx

	EXTI->IMR|=GPIO_Pin;					      //开启line BITx上的中断(如果要禁止中断，则反操作即可)
	if(TRIM&0x01)EXTI->FTSR|=GPIO_Pin;	//line BITx 事件下降沿触发
	if(TRIM&0x02)EXTI->RTSR|=GPIO_Pin;	//line BITx 事件上升沿触发
} 
