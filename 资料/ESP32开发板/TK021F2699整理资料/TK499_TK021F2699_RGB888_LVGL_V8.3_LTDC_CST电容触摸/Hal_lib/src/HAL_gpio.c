...
*
* <h2><center>&copy; COPYRIGHT 2022 HOLOCENE</center></h2>
*/ 
...
void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_InitStruct)
{
  ...
  if((GPIO_InitStruct->GPIO_Pin>>16)&&(GPIOx==GPIOE))  //GPIOE 16~23 bit	
  {
...
void GPIO_SetBits(GPIO_TypeDef* GPIOx, uint32_t GPIO_Pin)
{
  ...
  if(GPIO_Pin>=GPIO_Pin_16)
	{
		if(GPIO_Pin&0xffff)GPIOx->BSRR = GPIO_Pin&0xffff;
  ...
  assert_param(IS_GPIO_AF(GPIO_AF));
	...
	if(GPIO_Pin>>16)  //GPIO 16~23 bit
	{
		...
		for(i=0;i<8;i++)
		{
			if(temp&0x01)
			{
				GPIOE->AFRH_EXT &= ~((uint32_t)0xF << ((uint32_t)(i<<2)));     //AF configuration takes up half a byte, to x4,i.e. <<2
				GPIOE->AFRH_EXT |= ((uint32_t)GPIO_AF << ((uint32_t)(i<<2)));
			}
			temp = temp>>1;
		}
	}
	if(GPIO_Pin&0XFF00)  //GPIO 8~15 bit
		{
			temp = GPIO_Pin>>8;
			for(i=0;i<8;i++)
			{
