...
>
> **变更记录**:
> - 2026-07-12: v1.0 设计审批通过
> - 2026-07-12: v1.1 实现完成，更新文档以反映实际实现细节
...
main.c (app_main)
  ...
  │     ├── DNS 劫持服务器 (UDP 53, 所有域名 → 192.168.4.1)
  │     ├── Captive Portal (直接返回 HTML, 非 302 重定向)
...
| HTTP Server | `esp_http_server` | 轻量级，4KB 栈，8 个 URI handler，配网后 `httpd_stop()` 销毁 |
| **DNS 劫持** | **lwip UDP socket (端口 53)** | **把所有域名解析到 192.168.4.1，让手机 Captive Portal 探测请求到达 HTTP Server** |
| **Captive Portal** | **直接返回 HTML** | **探测路径直接返回配网页面 HTML，不使用 302 重定向** |
| Wi-Fi 扫描 | `esp_wifi_scan_start()` | APSTA 模式下 AP 频道扫描周边 Wi-Fi |

### 4.1.1 DNS 劫持服务器 (实现关键)

手机连上热点后，Captive Portal 探测流程是：DNS 查询 → HTTP 请求 → 判定。如果 AP 模式下没有 DNS 服务器，手机无法解析探测域名，导致探测请求根本到不了 HTTP Server。

**实现方案**：在 `web_portal_start()` 中创建一个轻量 UDP DNS 服务器（lwip socket, 端口 53），接收所有 DNS 查询并返回 `192.168.4.1` 作为 A 记录。

```c
static void dns_hijack_task(void *pv) {
    // 创建 UDP socket 绑定端口 53
    // 循环接收 DNS 查询，构造响应:
    //   - QR=1 (response)
    //   - ANCOUNT=1
    //   - Answer: 压缩指针 → TYPE A → CLASS IN → TTL 60s → 192.168.4.1
}
```

- Task 栈大小：3072 bytes
- DNS 响应 TTL：60 秒
- 配网成功后 `close(s_dns_sock)` 关闭，Task 自动退出

### 4.1.2 Captive Portal 探测响应 (实现关键)

原始设计使用 `302 Found` 重定向到 `/`，但真机调试发现很多手机的 Captive Portal mini-browser **不跟随 302 重定向**，导致弹出空白页面。

**修正方案**：所有探测路径直接返回配网页面 HTML 内容：
...
| GET | `/api/scan` | 返回 JSON 格式 Wi-Fi 列表 (最多 20 个) |
| POST | `/api/connect` | 接收 `{ssid, password}` → 保存 → 尝试连接 |
| GET | `/api/status` | 返回连接状态 (idle/connecting/connected/failed) |
| GET | `/generate_204` | Android Captive Portal 探测 → **直接返回 HTML** |
| GET | `/hotspot-detect.html` | iOS Captive Portal 探测 → **直接返回 HTML** |
| GET | `/ncsi.txt` | Windows Captive Portal 探测 → **直接返回 HTML** |
...
### 4.5 Captive Portal 完整拦截链

配网页面自动弹出依赖两层拦截：

**第一层 — DNS 劫持 (UDP 53)**
- 手机连接热点后，DHCP 分配 DNS 为 192.168.4.1
- 手机发起 DNS 查询（如 `clients3.google.com`）
- DNS 劫持服务器返回 192.168.4.1
- 手机 HTTP 探测请求到达我们的 HTTP Server

**第二层 — HTTP 探测路径直接返回 HTML**
- **Android** — GET `/generate_204` → 直接返回配网页面 HTML
- **iOS** — GET `/hotspot-detect.html` → 直接返回配网页面 HTML
- **Windows** — GET `/ncsi.txt` → 直接返回配网页面 HTML

> **设计变更说明**：原设计使用 302 重定向，但真机调试发现多数手机的 Captive Portal mini-browser 不跟随 302，导致空白页。改为直接返回 HTML 解决。

### 4.6 HTTP Server 内存配置

```c
httpd_config_t config = HTTPD_DEFAULT_CONFIG();
config.stack_size = 4096;           // 最小栈
config.max_uri_handlers = 8;        // 7 个端点 + 1 余量
config.lru_purge_enable = true;     // 自动清理旧连接
config.recv_wait_timeout = 5;       // 快速超时回收
```

> **sdkconfig 配置**：
> - `CONFIG_HTTPD_MAX_REQ_HDR_LEN=2048` (默认 512 太小，手机浏览器 User-Agent 等头部超限导致 "Header Field are too long" 错误)
...
| `sdkconfig.defaults` | 加 `CONFIG_ESP_WIFI_ENABLED=y` 等 Wi-Fi 支持, HTTPD header 增至 2048 |
...
> **实现变更说明**：原始设计设为 512 字节，但真机调试发现手机浏览器 User-Agent / Accept 等头部轻松超过 512 字节，导致 "Header Field are too long" 错误。增大到 2048/1024 后问题解决。
