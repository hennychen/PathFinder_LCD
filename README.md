...
```
                   ...
                   ▼                         ▼
     ┌────────────────────┐        ┌────────────────────┐
     │   板 A (EMOTE)      │        │   板 B (Tracker)   │
     │   MESH_ROLE_ROOT    │◄──────►│   MESH_ROLE_CHILD  │
     │   连接路由器         │  无线   │   加入 Mesh 网络     │
     └─────────┬──────────┘        └────────────────────┘
               │
               │ BLE GATT Notify
...
B板向A板发送数据时自动选择最佳通道（见 [`comm_link.c`](PathFinder_Tracker/main/comm/comm_link.c)）：

| 优先级 | 通道 | 延迟 | 可靠性 | 用途 |
|--------|------|------|--------|------|
| **1** | ESP-NOW | <1ms | 不可靠（无重传） | 角度/状态/心跳（小包 ≤250B） |
| **2** | ESP-WIFI-MESH P2P | 8-12ms | 可靠 | 人脸信息/大帧数据 |
| **3** | UART 物理串口 | 即时 | 始终可用 | 无线全失败时的兜底 |

- **默认部署：零通信接线**，两板均通过无线自动组网（ESP-Mesh + ESP-NOW）
- **消息类型**：角度数据、追踪状态、人脸信息、舵机控制、模式切换、心跳
- **消息帧格式**：`[MSG_TYPE(1)][SEQ(1)][PAYLOAD_LEN(1)][PAYLOAD(0-246)][CRC8(1)]`（≤250B兼容 ESP-NOW）
- **共享配置**：两板 `MESH_ID={0x18,0xFE,0x34,0x00,0x00,0x01}`，`MESH_AP_PASSWD="pathfinder_mesh_2026"`，`MESH_MAX_LAYER=2`（见 [`mesh_protocol.h`](PathFinder_Tracker/main/comm/mesh_protocol.h)）

### A板↔B板 接线方案

#### 方案一：无线组网（默认，零接线） ✅

**前提**：两板在同一 2.4GHz Wi-Fi 环境下，且 A板已通过 BLE/Web Portal 配网写入路由器 SSID/密码。

```
     A板 (EMOTE)                         B板 (Tracker)
     ┌──────────┐                        ┌──────────┐
     │ ROOT 节点 │   ◄─ ESP-WIFI-MESH ──► │ CHILD 节点 │
     │ 连接路由器 │   ◄─    ESP-NOW     ──► │ 加入 Mesh  │
     └──────────┘                        └──────────┘
         仅需给两板独立供电，无需任何信号线连接
```

**启动流程**：
1. A板上电 → BLE/Web 配网成功 → 自动启动 Mesh ROOT
2. B板上电 → `mesh_node_init(MESH_ROLE_CHILD)` → 自动搜索 ROOT → 加入网络
3. ESP-NOW 附着 Mesh 信道，低延迟通道可用

#### 方案二：联合供电（推荐供电拓扑）

使用单个 USB-C PD 5V/5A 适配器从 B板进电，再从 B板的 5V/VIN 和 GND 引脚拉杜邦线到 A板：
...
| 连线 | 起点 (B板) | 终点 (A板) | 作用 |
|------|-----------|-----------|------|
| **5V** | B板 5V/VIN | A板 5V/VIN | 供电共享 |
| **GND** | B板 GND | A板 GND | 共地参考（必需） |

**供电要点**：
- 电脑 USB 口（≤900mA）仅限烧录，不可带载运行
- 舵机与 WS2812 必须接入 B板 5V 总线，不可依赖 ESP32 芯片 3.3V 输出
- B板峰值电流可达 3-5A（舵机启动），必须使用 5V/3A+ 或 5V/5A PD 适配器
- 两板即使各自独立 USB 供电，也必须连 GND 线共地（否则 UART/射频不稳定）

#### 方案三：UART 物理备用通道（可选）

仅在无线失效或无路由器环境使用，**需要交叉连接 TX↔RX**：

```
     A板 (PathFinder_EMOTE)              B板 (PathFinder_Tracker)
     ┌──────────────────┐                ┌──────────────────┐
     │   GPIO ?? (TX)   ├───────────────►│   GPIO20 (RX)    │
     │                  │                │                  │
     │   GPIO ?? (RX)   │◄───────────────┤   GPIO19 (TX)    │
...
| 连线 | B板 GPIO | → | A板 GPIO | 说明 |
|------|----------|---|----------|------|
| B.TX → A.RX | GPIO19 | → | A板待定 RX | 必须交叉 |
| B.RX ← A.TX | GPIO20 | ← | A板待定 TX | 必须交叉 |
| 共地 | GND | ↔ | GND | 必须 |

**帧格式**：`[HEADER 0xAA][CMD][LEN][DATA[LEN]][CRC8][TAIL 0x55]`（见 [`drv_uart_comm.c`](PathFinder_Tracker/main/drivers/drv_uart_comm.c)）

> ⚠️ **A板 UART 接收驱动尚未实现**：目前B板有完整的发送实现，但A板端仅通过无线 (ESP-NOW/Mesh) 接收数据。如需启用 UART 备用通道，需在 A板 补齐 UART 接收驱动。
>
> ⚠️ **GPIO19/20 为 USB-JTAG 引脚**：B板已禁用 USB-CDC 控制台后才能用作 UART1，否则会与 USB 调试冲突。
...
│               │    ┌──── MG90S 舵机 ──────────────────────────┐
│               │    │                                          │
│  GPIO 47 ─────┼────┼──── 橙色信号线 ──► Pan (水平轴)          │
│  GPIO 14 ─────┼────┼──── 橙色信号线 ──► Tilt (垂直轴)         │
│               │    │    红线 ──────────► VCC (5V)              │
│               │    │    棕线 ──────────► GND                   │
│               │    │                                          │
│               │    └──────────────────────────────────────────┘
...
│               │
│               │    ┌──── UART 板间通信 (可选备用 → 板 A) ─────┐
│  GPIO 19 ─────┼────┼──── TX ────────► 板 A RX (交叉连接)      │
│  GPIO 20 ─────┼────┼──── RX ────────◄ 板 A TX (交叉连接)      │
│               │    │    115200 bps  (默认走无线, 仅在无线失效) │
│               │    └──────────────────────────────────────────┘
└───────────────┘
```
...
| 模块 | 文件 | 功能 |
|------|------|------|
| Mesh 节点 | `comm/mesh_node.c/h` | ESP-WIFI-MESH CHILD 节点（板 B），自动搜索 ROOT 并加入网络 |
| ESP-NOW | `comm/mesh_espnow.c/h` | ESP-NOW 低延迟通道，附着 Mesh 信道 |
| 通信链路 | `comm/comm_link.c/h` | 三级 fallback 调度器（ESP-NOW > Mesh > UART） + 心跳上报 |
| 接收处理 | `main.c::handle_board_a_msg()` | 处理 A 板下发的 MSG_SERVO_CTRL/MSG_MODE_SWITCH 指令 |
| 协议 | `comm/mesh_protocol.c/h` | 共享消息协议（与板 A 一致） |
| Web Viewer 互斥 | `main.c::app_main()` | Mesh 启动成功时自动跳过 `web_viewer_start()`，避免 WiFi 占用冲突 |

> **CHILD 节点配置说明**：B板作为 CHILD **不需要路由器 SSID/密码**，仅需共享的 `MESH_ID` 和 `MESH_AP_PASSWD` 即可搜索并加入 ROOT 构建的 Mesh 网络。原先注释「Mesh需要路由器SSID/PASSWORD配置」为误解，已修复。
