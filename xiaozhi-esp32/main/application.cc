...
/* PathFinder Tracker board 转发接口（弱符号，非 tracker 板时链接器自动填 NULL） */
extern "C" __attribute__((weak)) void pathfinder_forward_subtitle(const char *text);
extern "C" __attribute__((weak)) void pathfinder_forward_emotion(const char *emotion);
...
void Application::InitializeProtocol() {
    ...
    protocol_->OnIncomingAudio([this](std::unique_ptr<AudioStreamPacket> packet) {
        static uint32_t s_audio_pkt_count = 0;
        static int64_t s_last_log_time = 0;
        s_audio_pkt_count++;
        int64_t now = esp_timer_get_time();
        if (now - s_last_log_time >= 2000000) {  // 每2秒打印一次
            ESP_LOGI(TAG, "Incoming audio: %lu pkts, state=%d, pkt_sz=%zu",
                     (unsigned long)s_audio_pkt_count, GetDeviceState(), packet->payload.size());
            s_last_log_time = now;
        }
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (strcmp(state->valuestring, "start") == 0) {
                ...
                if (cJSON_IsString(text)) {
                    ...
                    Schedule([display, message = std::string(text->valuestring),
                        ...
                        /* 转发 AI 回复字幕到 PathFinder A板 */
                        if (pathfinder_forward_subtitle) {
                            pathfinder_forward_subtitle(message.c_str());
                        }
                    });
                }
            }
            ...
            if (cJSON_IsString(emotion)) {
                Schedule([display, emotion_str = std::string(emotion->valuestring)]() {
                    ...
                    /* 转发 LLM 情感标签到 PathFinder A板 */
                    if (pathfinder_forward_emotion) {
                        pathfinder_forward_emotion(emotion_str.c_str());
                    }
                });
            }
