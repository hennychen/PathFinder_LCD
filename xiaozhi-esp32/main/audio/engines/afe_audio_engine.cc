...
bool AfeAudioEngine::Initialize(AudioCodec* codec, int frame_duration_ms, srmodel_list_t* models_list) {
    ...
    // 在内部 RAM 紧张时，将 AFE 任务栈分配到 PSRAM
    ESP_LOGI(TAG, "AFE task pre-alloc — free internal: %u, free PSRAM: %u",
             (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL),
             (unsigned)heap_caps_get_free_size(MALLOC_CAP_SPIRAM));

    constexpr size_t kAfeTaskStackSize = 4096;
    processing_task_stack_ = static_cast<StackType_t*>(
        heap_caps_malloc(kAfeTaskStackSize, MALLOC_CAP_SPIRAM));
    processing_task_buffer_ = static_cast<StaticTask_t*>(
    ...
    if (processing_task_stack_ != nullptr && processing_task_buffer_ != nullptr) {
        processing_task_ = xTaskCreateStatic([](void* arg) {
            auto* engine = static_cast<AfeAudioEngine*>(arg);
            engine->ProcessingTask();
            vTaskDelete(nullptr);
        }, "audio_afe", kAfeTaskStackSize, this, 3,
            processing_task_stack_, processing_task_buffer_);
    }

    if (processing_task_ == nullptr) {
        ESP_LOGE(TAG, "Failed to create AFE processing task (stack=%p, tcb=%p, free_internal=%u)",
                 processing_task_stack_, processing_task_buffer_,
                 (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL));
        if (processing_task_stack_) { heap_caps_free(processing_task_stack_); processing_task_stack_ = nullptr; }
        if (processing_task_buffer_) { heap_caps_free(processing_task_buffer_); processing_task_buffer_ = nullptr; }
        afe_iface_->destroy(afe_data_);
        afe_data_ = nullptr;
        afe_iface_ = nullptr;
        return false;
