...
static esp_netif_t      *s_netif_sta = NULL;  /* STA netif for DHCP restart on ROOT */
                                ...
                                int32_t event_id, void *event_data)
{
    ...
    case MESH_EVENT_PARENT_CONNECTED: {
        ...
        /* ROOT must restart DHCP client to obtain IP + DNS from router.
         * Without this, the ROOT has WiFi link but no IP address, causing
         * DNS resolution failures (getaddrinfo returns 202/EAI_NONAME). */
        if (esp_mesh_is_root() && s_netif_sta) {
            ESP_LOGI(TAG, "ROOT: restarting DHCP client to get IP/DNS");
...
esp_err_t mesh_node_start_root_after_wifi(const char *ssid, const char *password)
{
    ...
    /* Create mesh-optimized netifs (STA + AP) using the official ESP-IDF API.
     * This is different from esp_netif_create_default_wifi_sta/ap — it creates
     * netifs specifically designed for mesh operation.
     *
     * xiaozhi's WifiManager::Initialize() calls esp_wifi_init() but does NOT
     * create netifs, so we must create them here. */
    esp_netif_t *netif_sta = NULL;
    if (esp_netif_get_handle_from_ifkey("WIFI_STA_DEF") == NULL) {
        esp_netif_create_default_wifi_mesh_netifs(&netif_sta, NULL);
        ESP_LOGI(TAG, "Created mesh netifs (STA+AP)");
    } else {
        /* Netif already exists (may happen on re-init) — get the handle */
        netif_sta = esp_netif_get_handle_from_ifkey("WIFI_STA_DEF");
    }
    s_netif_sta = netif_sta;  /* Save for DHCP restart in event handler */

    /* Start WiFi WITHOUT setting mode or connecting — following the official
     * ESP-IDF mesh example exactly. Mesh manages WiFi mode internally.
     *
     * Key lessons learned (DO NOT do these):
     * - DO NOT call esp_wifi_set_mode(): prevents Mesh from switching to STA+AP
     * - DO NOT call esp_wifi_connect(): pre-connection causes infinite scan loop
     * - DO NOT use WIFI_STORAGE_RAM: Mesh needs FLASH storage for internal state
     */
    esp_wifi_set_storage(WIFI_STORAGE_FLASH);
    esp_wifi_start();
    ESP_LOGI(TAG, "WiFi started (default mode, not connected)");
    ...
    /* Configure mesh — following official ESP-IDF mesh example */
    mesh_cfg_t mesh_cfg = MESH_INIT_CONFIG_DEFAULT();
    uint8_t mesh_id[6] = MESH_ID_BYTES;
    memcpy((uint8_t *)&mesh_cfg.mesh_id, mesh_id, 6);
    mesh_cfg.channel = MESH_CHANNEL;
    ...
    /* Set max layer and let ROOT election happen naturally.
     * DO NOT use esp_mesh_fix_root(true) — it causes Mesh to skip WiFi
     * connection and enter an infinite [FAIL]root:0 scan loop.
     * Natural election: the first node to find the router becomes ROOT. */
    esp_mesh_set_max_layer(MESH_MAX_LAYER);
    esp_mesh_set_vote_percentage(1);
    esp_mesh_set_xon_qsize(128);
    /* Disable mesh power-save for simpler debugging */
    esp_mesh_disable_ps();
